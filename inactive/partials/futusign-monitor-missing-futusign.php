 ...
 * @since    0.3.0
 */
function futusign_monitor_missing_futusign() {
	$is_installed = Futusign_Monitor::is_plugin_installed( 'futusign' );
	$target = false;
	$action = __('Install', 'futusign_monitor');
	if ( current_user_can( 'install_plugins' ) ) {
		if ( $is_installed ) {
			$action = __('Activate', 'futusign_monitor');
	...
	<div class="notice error is-dismissible">
		<p><strong>futusign Monitor</strong> <?php esc_html_e('depends on the last version of futusign to work!', 'futusign_monitor' ); ?></p>
...
futusign_monitor_missing_futusign();
